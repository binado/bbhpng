...
def get_covariance(
    model, FLAG_NOCROSS_SPEC=False, RSD=False, multipoles=[True, True, True]
):
    print("Getting Covariance \n")
    if not RSD:
        return get_covariance_real(model, FLAG_NOCROSS_SPEC=FLAG_NOCROSS_SPEC)
    else:
        return get_covariance_rsd(
            model, FLAG_NOCROSS_SPEC=FLAG_NOCROSS_SPEC, multipoles=multipoles
        )


def get_covariance_real(
    model, FLAG_NOCROSS_SPEC=False
):  ### no-cross not really used here ### I think it it...
    lenk = model.len_k
    ...
    for idx_cov1 in range(model.ntracers):
        for idx_cov2 in range(idx_cov1, model.ntracers):
            if FLAG_NOCROSS_SPEC == 1 and idx_cov2 != idx_cov1:
                continue  ### just skip those if no cross

            for idx_cov3 in range(model.ntracers):
                for idx_cov4 in range(idx_cov3, model.ntracers):
                    if FLAG_NOCROSS_SPEC == 1 and idx_cov4 != idx_cov3:
                        continue
                    ...
                    for idx_k in range(lenk):
                        ...
                        if (
                            ...
                            exit(1)
                        CABCD = (
                            PAC_real[idx_k] * PBD_real[idx_k]
                            + PAD_real[idx_k] * PBC_real[idx_k]
                        ) / Mk
...
def cov_rsd_value(ell, ell_prime, Nk, PAC, PBD, PAD, PBC):
    ...
    for index1, ell1 in enumerate(ell_array):
        for index2, ell2 in enumerate(ell_array):
            ...
            for index3, ell3 in enumerate(
                [0, 2, 4, 6, 8]
            ):  ### Important: here it is fixed by triangle ineq. in Wigner!
                summing += (
                    (2.0 * ell3 + 1.0)
                    * wigner_3j(ell1, ell2, ell3, 0, 0, 0) ** 2.0
                    * wigner_3j(ell, ell_prime, ell3, 0, 0, 0) ** 2.0
                    * (
                        PAC[index1] * PBD[index2]
                        + (-1) ** (ell_prime + ell2) * PAD[index1] * PBC[index2]
                    )
                )
    ...
    return summing * (2.0 * ell + 1.0) * (2.0 * ell_prime + 1.0) / Nk


def get_covariance_rsd(
    model, FLAG_NOCROSS_SPEC=False, multipoles=[True, True, True]
):  ## todo nocross
    ell_array = np.array([0, 2, 4])[multipoles]
    lenk = model.len_k
    lenell = len(ell_array)

    if not FLAG_NOCROSS_SPEC:
    ...
    for Aidx in range(model.ntracers):
        for Bidx in range(Aidx, model.ntracers):
            if FLAG_NOCROSS_SPEC == 1 and Aidx != Bidx:
                continue  ### just skip those if no cross

            for Cidx in range(model.ntracers):
                for Didx in range(Cidx, model.ntracers):
                    if FLAG_NOCROSS_SPEC == 1 and Cidx != Didx:
                        continue  ### just skip those if no cross
                    ...
                    if multipoles[0]:
                        PAC_0 = model.Pdata[
                            ACidx * lenk * lenell + 0 * lenk : (ACidx) * lenk * lenell
                            + 0 * lenk
                            + lenk
                        ]
                        PBD_0 = model.Pdata[
                            BDidx * lenk * lenell + 0 * lenk : (BDidx) * lenk * lenell
                            + 0 * lenk
                            + lenk
                        ]
                        PAD_0 = model.Pdata[
                            ADidx * lenk * lenell + 0 * lenk : (ADidx) * lenk * lenell
                            + 0 * lenk
                            + lenk
                        ]
                        PBC_0 = model.Pdata[
                            BCidx * lenk * lenell + 0 * lenk : (BCidx) * lenk * lenell
                            + 0 * lenk
                            + lenk
                        ]
                    ...
                    if multipoles[1]:
                        ...
                        PAC_2 = model.Pdata[
                            ACidx * lenk * lenell + temp * lenk : (ACidx)
                            * lenk
                            * lenell
                            + temp * lenk
                            + lenk
                        ...
                        PBD_2 = model.Pdata[
                            BDidx * lenk * lenell + temp * lenk : (BDidx)
                            * lenk
                            * lenell
                            + temp * lenk
                            + lenk
                        ...
                        PAD_2 = model.Pdata[
                            ADidx * lenk * lenell + temp * lenk : (ADidx)
                            * lenk
                            * lenell
                            + temp * lenk
                            + lenk
                        ...
                        PBC_2 = model.Pdata[
                            BCidx * lenk * lenell + temp * lenk : (BCidx)
                            * lenk
                            * lenell
                            + temp * lenk
                            + lenk
                    ...
                    for index1, ell1 in enumerate(ell_array):
                        for index2, ell2 in enumerate(ell_array):
                            for idx in range(lenk):
                                ...
                                if idx == lenk - 1:
                                    Deltak = (
                                        model.k_loc[idx] - model.k_loc[idx - 1]
                                    )  ### fixing it for last index...
                                else:
                                    Deltak = model.k_loc[idx + 1] - model.k_loc[idx]

                                CABCD = cov_rsd_value(
                                    ell1, ell2, model.Nk_loc[idx], PAC, PBD, PAD, PBC
                                )
                                # CABCD = cov_rsd_value(ell1, ell2, k, Deltak, model.Vbox, PAC, PBD, PAD, PBC)
                                covidx1 = (
                                    ABidx * len(ell_array) * lenk + index1 * lenk + idx
                                )
                                covidx2 = (
                                    CDidx * len(ell_array) * lenk + index2 * lenk + idx
                                )
