...
def fisher(model, analytic_bias = True, nocross_spec = False, RSD = False, multipoles = [True, True, True], nbarvalue = 1e-4, V1440_factor = 1, includeAs = False, assymetric_kmax = []):
    """
...
#    model = modellib.set_model(ntracers, nbar_value = nbar_value, RSD = RSD, V1440_factor = V1440_factor, includeAs = includeAs)
    ...
    derivatives = model.get_derivatives(analytic_bias = analytic_bias, FLAG_NOCROSS_SPEC = nocross_spec)
  ...
  #    print ('deriv ', i, derivatives[i])
    ...
    cov = covariance.get_covariance(model, FLAG_NOCROSS_SPEC = nocross_spec, RSD = RSD, multipoles = multipoles)
    ...
    if (RSD): nmultipoles = sum(multipoles)
    else:     nmultipoles = 1
    #exit(1)
        ...
        #for j in range(i, len_param):
            for idx_Pspec1 in range(idxmax): 
                ...
                for idx_Pspec2 in range(idx_Pspec1,idxmax):
                    if (idx_Pspec1 != idx_Pspec2 and nocross_spec == True): continue
                    for idx_Pspec3 in range(idxmax): 
                        for idx_Pspec4 in range(idx_Pspec3,idxmax):
                            if (idx_Pspec3 != idx_Pspec4 and nocross_spec == True): continue

                            if (nocross_spec == False):
                            ...
                            for ell1 in range(nmultipoles):
                                for ell2 in range(nmultipoles):
                                    for idxk in range(len_k):
                                        if (assymetric_kmax != []):
                                          if (model.k_loc[idxk] > assymetric_kmax[idx_ab]):
#                                            print ("Skipping: ", model.k_loc[idxk], assymetric_kmax[idx_ab], idx_ab)
                                #            exit(1)
                                            continue
                                          if (model.k_loc[idxk] > assymetric_kmax[idx_cd]):
 #                                           print ("Skipping: ", model.k_loc[idxk], assymetric_kmax[idx_cd], idx_cd)
                                 #           exit(1)
                                            continue
                                        idx1 = (idx_ab)*len_k*nmultipoles+ell1*len_k+idxk
                                        idx2 = (idx_cd)*len_k*nmultipoles+ell2*len_k+idxk
                                        #print (idx1, idx2)
                                        Fmatrix[i, j] += (derivatives[i][idx1] *
                                                            derivatives[j][idx2] *
                                                            covm1[idx1, idx2])
                                        if (derivatives[i][idx1] != derivatives[i][idx1]):
                                          masking = model.Pdata > 0
                                          print("Weird: ",i, derivatives[i][masking], derivatives[i][~masking])
...
#                                            covm1[idx_Pspec1*len_k+idxk, idx_Pspec2*len_k+idxk])
    ...
    priors = np.diag(1. / sigma2prior_array[sigma2prior_array != 0])
    Fmatrix += priors
