...
class Model:
    ...
    def __init__(self, cosmo_parameters, bias_parameters, stoch_parameters, tracers_shot_noise, 
                 kmin, kmax, Vbox, z, p1loop=1., noise_amplitude=0, eps=0.04, 
                 RSD = False, multipoles = [True, True, True],
                 open_precomputed_files = True, save_precomputed_files = False):
        """
        ...
        self.pk_mult_fid, self.knormh_fid, self.fz = self.compute_cosmo_func(self.omega_cdm_fid, self.h_fid, self.As_fid, openfile = self.open_precomputed_files, savefile = self.save_precomputed_files)
        #print ("pkmult", len(self.pk_mult_fid))
        #exit(1)

        # Initialize model data
        self.Pdata = self.get_data(self.pk_mult_fid, self.fz, self.all_parameters_values)
        ...
        if (self.RSD == False): self.analytic_deriv = self.get_analytic_deriv()
        else:                   self.analytic_deriv = self.get_analytic_deriv_rsd()
    ...
    def _initialize_parameters(self):
        ...
        self.all_parameters_values = [props['value'] for props in self.cosmo_parameters.values()]
        self.active_parameters = [props for props in self.cosmo_parameters.values() if props.get('active', False)]
        self.active_priors = [props['priorsigma'] for props in self.cosmo_parameters.values() if props.get('active', False)]
        ...
        for tracer_i in self.bias_parameters:
          ...
          temp_active = [props for props in tracer_i.values() if props.get('active', False)]
          temp_prior = [props['priorsigma'] for props in tracer_i.values() if props.get('active', False)]
          self.all_parameters = np.concatenate((self.all_parameters, temp_param))
          self.all_parameters_values = np.concatenate((self.all_parameters_values, temp_value))
          self.active_parameters = np.concatenate((self.active_parameters, temp_active))
    ...
    def set_ks(self):
        ...
        """
        k = np.array([0.00436332, 0.00690733, 0.01191144, 0.01681231, 0.02110734, 0.02586348,
           0.03066663, 0.0351514,  0.03980047, 0.04462564, 0.04929755, 0.05408208,
           0.05878054, 0.06337915, 0.06796408, 0.07265316, 0.0773745,  0.08206283,
           0.08669998, 0.09151402, 0.09614883, 0.10081397, 0.10552219, 0.11013738,
           0.11481222, 0.11955234, 0.12421079, 0.12884013, 0.13356501, 0.13828716,
           0.14306583, 0.14776735, 0.15239276, 0.15710744, 0.16179708, 0.16642278,
           0.17110649, 0.17579253, 0.18052566, 0.18520921, 0.18987729, 0.19453861,
           0.19923207, 0.20393509, 0.20863545, 0.21333136, 0.21805128, 0.2227085,
           0.22738634, 0.23205931, 0.2367079,  0.24136284, 0.24605665, 0.25077188,
           0.25547436, 0.26018536, 0.26486027, 0.26955128, 0.2742491,  0.27894068,
           0.2836418,  0.2883313,  0.29297578, 0.2976653 ])

        k = np.array(k, dtype=np.float64)
        deltak = np.diff(k)
        deltak = np.append(deltak, deltak[-1])  # Ensure same length as k
        Nk = k**2 * deltak * self.Vbox / (2. * np.pi**2.)
    ...
    def get_data(self, pk_mult, fz, args): #both ST and MT
        ...
        omega_cdm, h, bias_set_full, stoch_set_full = self.get_cosmo_bias_stoch_set(args)
        ...
        for tracer_A in range(self.ntracers):
            for tracer_B in range(tracer_A, self.ntracers):
                ...
                if (self.RSD == False):
                  spectrum = self.spec_model_nlin(pk_mult, h, fz, bias_setA, bias_setB, stoch_setAB,
                                             self.nbarsample[tracer_A], self.nbarsample[tracer_B])
                else:
                  spectrum = self.spec_model_nlin_rsd(pk_mult, h, fz, bias_setA, bias_setB, stoch_setAB,
                                             self.nbarsample[tracer_A], self.nbarsample[tracer_B])
    ...
    def get_cosmo_bias_stoch_set(self, args, FLAG_NOCROSS_SPEC = False):
        ...
        for idx_tracers in range(self.ntracers):
          bias_set.append(args[3+idx_tracers*self.nbias:3+(idx_tracers+1)*self.nbias])
        stochastic_set = []

        if (FLAG_NOCROSS_SPEC): idxmax = self.ntracers
        else:                   idxmax = self.ntracerssum

        for idx_tracers in range(idxmax):
          stochastic_set.append(args[3+self.ntracers*self.nbias+idx_tracers*self.nstoch:3+self.ntracers*self.nbias+(idx_tracers+1)*self.nstoch])
...
#                temp = np.zeros((self.ntracers * self.len_k * np.sum(self.multipoles)))
                ...
                for idx1 in range(self.ntracers):
                    ...
                    temp = derivatives[i][idx12*nmultipoles*self.len_k:(idx12+1)*nmultipoles*self.len_k]
    ...
    def spec_model_nlin(self, pk_mult_loc, h_loc, fz, bias_setA, bias_setB, stoch_set, nbarA_loc, nbarB_loc):
        """
        ...
        result = (
            ...
            self.p1loop*(bNabla2_A * b1_B + bNabla2_B * b1_A) * pk_mult_loc[10] / knormh_loc**2 +
            ...
            self.p1loop*(b1_A * (bG2_B + 0.4 * bGamma3_B) + b1_B * (bG2_A + 0.4 * bGamma3_A)) * pk_mult_loc[6] +
        ...
        noise_array = np.array([1. + (np.random.normal(0, self.noise_amplitude, 1)[0] if self.noise_amplitude else 0.) for _ in range(len(stochastic_term))])

        return result * noise_array + stochastic_term

    def spec_model_nlin_rsd(self, pk_mult_loc, h_loc, fz, bias_setA, bias_setB, stoch_set, nbarA_loc, nbarB_loc):
        """
        ...
        b1_A, b2_A, bG2_A, bGamma3_A, cct_2_0_A, cct_2_2_A, cct_2_4_A, cct_4_4_A, cct_4_6_A = bias_setA
        b1_B, b2_B, bG2_B, bGamma3_B, cct_2_0_B, cct_2_2_B, cct_2_4_B, cct_4_4_B, cct_4_6_B = bias_setB
#        print ("\n\n\n values for A, B:", cct_2_2_B, cct_2_2_A )
        ...
        Pct_0_AB = self.p1loop*(
            +(35. * (fz + 3. * b1_A) * cct_2_0_B
              + 7. * (3. * fz + 5. * b1_A) * cct_2_2_B
              + 3. * (5. * fz + 7. * b1_A) * (cct_2_4_B + k2ratio * cct_4_4_B)
              + (35. / 21.) * (7. * fz + b1_A) * (cct_2_6_B + k2ratio * cct_4_6_B)
            ) * (1. / 105.) * pk_mult14_scaled
        )
    
        Pct_2_AB = self.p1loop*(
            +(7. * fz * cct_2_0_B
              + (6. * fz + 7. * b1_A) * cct_2_2_B
              + (5. * fz + 6. * b1_A) * (cct_2_4_B + k2ratio * cct_4_4_B)
              + (20. / 33.) * (28. * fz + 33. * b1_A) * (cct_2_6_B + k2ratio * cct_4_6_B)
            ) * (2. / 21.) * pk_mult14_scaled
        )
    
        Pct_4_AB = self.p1loop*(
            +(11. * fz * cct_2_2_B
              + (15. * fz + 11. * b1_A) * (cct_2_4_B + k2ratio * cct_4_4_B)
              + (15. / 13.) * (14. * fz + 13. * b1_A) * (cct_2_6_B + k2ratio * cct_4_6_B)
            ) * (8. / 385.) * pk_mult14_scaled
        )
    
        Pct_0_BA = self.p1loop*(
            +(35. * (fz + 3. * b1_B) * cct_2_0_A
              + 7. * (3. * fz + 5. * b1_B) * cct_2_2_A
              + 3. * (5. * fz + 7. * b1_B) * (cct_2_4_A + k2ratio * cct_4_4_A)
              + (35. / 21.) * (7. * fz + b1_B) * (cct_2_6_A + k2ratio * cct_4_6_A)
            ) * (1. / 105.) * pk_mult14_scaled
        )
    
        Pct_2_BA = self.p1loop*(
            +(7. * fz * cct_2_0_A
              + (6. * fz + 7. * b1_B) * cct_2_2_A
              + (5. * fz + 6. * b1_B) * (cct_2_4_A + k2ratio * cct_4_4_A)
              + (20. / 33.) * (28. * fz + 33. * b1_B) * (cct_2_6_A + k2ratio * cct_4_6_A)
            ) * (2. / 21.) * pk_mult14_scaled
        )
    
        Pct_4_BA = self.p1loop*(
            +(11. * fz * cct_2_2_A
              + (15. * fz + 11. * b1_B) * (cct_2_4_A + k2ratio * cct_4_4_A)
              + (15. / 13.) * (14. * fz + 13. * b1_B) * (cct_2_6_A + k2ratio * cct_4_6_A)
            ) * (8. / 385.) * pk_mult14_scaled
        )
    
        # Add stochastic terms
        Pst_0 = (cshot + cst_0_0 + self.p1loop*cst_0_2 * k2ratio + self.p1loop*cst_2_2 * fz * k2ratio / 3.) / nbarmix
        Pst_2 = self.p1loop*(2. * cst_2_2 * fz * k2ratio / 3.) / nbarmix
        Pst_4 = 0.
     
        # Compute multipoles
        monopole = (
            pk_mult_loc[15] + self.p1loop*pk_mult_loc[21]
            + (b1_A + b1_B) / 2. * (pk_mult_loc[16] + self.p1loop*pk_mult_loc[22])
            + b1_A * b1_B * (pk_mult_loc[17] + self.p1loop*pk_mult_loc[23])
            + self.p1loop*0.25 * b2_A * b2_B * pk_mult_loc[1]
            + self.p1loop*(b1_A * b2_B + b1_B * b2_A) / 2. * pk_mult_loc[30]
            + self.p1loop*(b2_A + b2_B) / 2. * pk_mult_loc[31]
            + self.p1loop*(b1_A * bG2_B + b1_B * bG2_A) / 2. * pk_mult_loc[32]
            + self.p1loop*(bG2_A + bG2_B) / 2. * pk_mult_loc[33]
            + self.p1loop*(b2_A * bG2_B + b2_B * bG2_A) / 2. * pk_mult_loc[4]
            + self.p1loop*bG2_A * bG2_B * pk_mult_loc[5]
            + self.p1loop*(bG2_A + 0.4 * bGamma3_A) * (b1_B * pk_mult_loc[7] + pk_mult_loc[8])
            + self.p1loop*(bG2_B + 0.4 * bGamma3_B) * (b1_A * pk_mult_loc[7] + pk_mult_loc[8])
        ) * h_loc**3 + Pct_0_AB + Pct_0_BA + Pst_0
...
#        exit(1)
        ...
        quadrupole = (
            pk_mult_loc[18] + self.p1loop*pk_mult_loc[24]
            + (b1_A + b1_B) / 2. * (pk_mult_loc[19] + self.p1loop*pk_mult_loc[25])
            + self.p1loop*b1_A * b1_B * pk_mult_loc[26]
            + self.p1loop*(b1_A * b2_B + b1_B * b2_A) / 2. * pk_mult_loc[34]
            + self.p1loop*(b2_A + b2_B) / 2. * pk_mult_loc[35]
            + self.p1loop*(b1_A * bG2_B + b1_B * bG2_A) / 2. * pk_mult_loc[36]
            + self.p1loop*(bG2_A + bG2_B) / 2. * pk_mult_loc[37]
            + self.p1loop*(bG2_A + 0.4 * bGamma3_A) * pk_mult_loc[9]
            + self.p1loop*(bG2_B + 0.4 * bGamma3_B) * pk_mult_loc[9]
        ) * h_loc**3 + Pct_2_AB + Pct_2_BA + Pst_2
    
        hexadecapole = (
            pk_mult_loc[20] + self.p1loop*pk_mult_loc[27]
            + self.p1loop*(b1_A + b1_B) / 2. * pk_mult_loc[28]
            + self.p1loop*b1_A * b1_B * pk_mult_loc[29]
            + self.p1loop*(b2_A + b2_B) / 2 * pk_mult_loc[38]
            + self.p1loop*(bG2_A + bG2_B) / 2. * pk_mult_loc[39]
        ) * h_loc**3 + Pct_4_AB + Pct_4_BA + Pst_4   

      
        if (self.multipoles[0] == False): 
          monopole = []
          #monopole = [0] * self.len_k 
        if (self.multipoles[1] == False): 
          quadrupole = []
        if (self.multipoles[2] == False): 
          hexadecapole = []
...
#    def compute_cosmo_func(self, omega_cdm, h_local, As_local, openfile = False, savefile = True):
    def compute_cosmo_func(self, omega_cdm, h_local, As_local, openfile = True, savefile = False):
        """
        ...
        def get_filename(omega_cdm, h_local, As_local):
          ...
          if   (As_local == AS_FID):       filename_return += 'As_0'  
          elif (As_local == AS_FID + 0.04*AS_FID): filename_return += 'As_p04'  
          elif (As_local == AS_FID + 0.08*AS_FID): filename_return += 'As_p08'  
          elif (As_local == AS_FID - 0.04*AS_FID): filename_return += 'As_m04'  
          elif (As_local == AS_FID - 0.08*AS_FID): filename_return += 'As_m08'  

          elif (As_local == AS_FID + 0.004*AS_FID): filename_return += 'As_p004'  
          elif (As_local == AS_FID + 0.008*AS_FID): filename_return += 'As_p008'  
          elif (As_local == AS_FID - 0.004*AS_FID): filename_return += 'As_m004'  
          elif (As_local == AS_FID - 0.008*AS_FID): filename_return += 'As_m008' 
          ...
          if   (h_local == H_FID):       filename_return += 'h_0'  
          elif (h_local == H_FID + 0.04*H_FID): filename_return += 'h_p04'  
          elif (h_local == H_FID + 0.08*H_FID): filename_return += 'h_p08'  
          elif (h_local == H_FID - 0.04*H_FID): filename_return += 'h_m04'  
          elif (h_local == H_FID - 0.08*H_FID): filename_return += 'h_m08'  

          elif (h_local == H_FID + 0.004*H_FID): filename_return += 'h_p004'  
          elif (h_local == H_FID + 0.008*H_FID): filename_return += 'h_p008'  
          elif (h_local == H_FID - 0.004*H_FID): filename_return += 'h_m004'  
          elif (h_local == H_FID - 0.008*H_FID): filename_return += 'h_m008'  
          ...
          if   (omega_cdm == OMEGA_CDM_FID):       filename_return += 'w_0'  
          elif (omega_cdm == OMEGA_CDM_FID + 0.04*OMEGA_CDM_FID): filename_return += 'w_p04'  
          elif (omega_cdm == OMEGA_CDM_FID + 0.08*OMEGA_CDM_FID): filename_return += 'w_p08'  
          elif (omega_cdm == OMEGA_CDM_FID - 0.04*OMEGA_CDM_FID): filename_return += 'w_m04'  
          elif (omega_cdm == OMEGA_CDM_FID - 0.08*OMEGA_CDM_FID): filename_return += 'w_m08'  

          elif (omega_cdm == OMEGA_CDM_FID + 0.004*OMEGA_CDM_FID): filename_return += 'w_p004'  
          elif (omega_cdm == OMEGA_CDM_FID + 0.008*OMEGA_CDM_FID): filename_return += 'w_p008'  
          elif (omega_cdm == OMEGA_CDM_FID - 0.004*OMEGA_CDM_FID): filename_return += 'w_m004'  
          elif (omega_cdm == OMEGA_CDM_FID - 0.008*OMEGA_CDM_FID): filename_return += 'w_m008'  
...
#          pk_mult_local = cosmo.get_pk_mult(kh_local, self.z, len(kh_local))
          ...
          # Wavenumber in 1/Mpc
          kh_local_unmasked = np.array(self.k_loc_unmasked * h_local, dtype=np.float64)
  
          # Initialize output
          cosmo.initialize_output(kh_local_unmasked, self.z, len(kh_local_unmasked))
          pk_mult_local = cosmo.get_pk_mult(kh_local_unmasked, self.z, len(kh_local_unmasked))
    ...
    def numerical_derivative(self, FUNC, active_idx, param_idx, eps, debug = False):
        ...
        if eps_rel == 0.:  # Handle zero-value parameters (e.g., stochastic terms)
        ...
        for shift in [2, 1, -1, -2]:  # Evaluate shifts at ±2ε and ±ε
            ...
            if self.all_parameters[param_idx].get("type") == 'cosmo':
                pk_mult_loc, _, fz = self.compute_cosmo_func(args[0], args[1], args[2], openfile = self.open_precomputed_files, savefile = self.save_precomputed_files)
            else:
        ...
        derivative_log = (
            -shifted_values[0] + 8 * shifted_values[1] - 8 * shifted_values[2] + shifted_values[3]
        ) / (12 * eps_rel) * f_x
...
#          exit(1)
    ...
    def analytical_derivative(self, FUNC, param_idx):
        ...
        omega_cdm, h, bias_set_full, stoch_set_full = self.get_cosmo_bias_stoch_set(self.all_parameters_values)

        # Initialize list to store spectra
        spectras = []
        if (self.RSD==True): len_ell = 3
        else: len_ell = 1
        ...
        for tracer_A in range(self.ntracers):
            for tracer_B in range(tracer_A, self.ntracers):
                ...
                if (paramtype == 'bias'):
                    ...
                    if (inA == False and inB == False):
                      spectrum = [0] * self.len_k * len_ell
                    elif (inA == False and inB == True):
                      spectrum = self.eval_derivative(paramname, 1, h, bias_setA, bias_setB, stoch_setAB, self.nbarsample[tracer_A], self.nbarsample[tracer_B])
                    elif (inA == True and inB == False):
                      spectrum = self.eval_derivative(paramname, 0, h, bias_setA, bias_setB, stoch_setAB, self.nbarsample[tracer_A], self.nbarsample[tracer_B])
                    elif (inA == True and inB == True):
                      prefactor = 2. if paramtype == 'bias' else 1.
                      spectrum = prefactor*np.array(self.eval_derivative(paramname, 0, h, bias_setA, bias_setB, stoch_setAB, self.nbarsample[tracer_A], self.nbarsample[tracer_B]))
                if (paramtype == 'stoch'):
                    Ais0 = tracer_A == self.all_parameters[param_idx].get('traceridx')[0]
                    Bis1 = tracer_B == self.all_parameters[param_idx].get('traceridx')[1]
                    if (Ais0 == True and Bis1 == True):
                      prefactor = 2. if paramtype == 'bias' else 1.
                      spectrum = prefactor*np.array(self.eval_derivative(paramname, 0, h, bias_setA, bias_setB, stoch_setAB, self.nbarsample[tracer_A], self.nbarsample[tracer_B]))
                    else:
                      spectrum = [0] * self.len_k * len_ell
    ...
    def get_analytic_deriv(self):
        ...
        c0, c2, h_loc, nbarmix, knormh_loc, knorm_loc = sp.symbols('c0 c2 h_loc nbarmix knormh_loc knorm_loc')
        ...
        expr = (
            ...
            (bNabla2_A * b1_B + bNabla2_B * b1_A) * pk_mult_symbolic[10] / knormh_loc**2 +
            0.5 * (b1_A * b2_B + b1_B * b2_A) * pk_mult_symbolic[2] +
            0.25 * b2_A * b2_B * pk_mult_symbolic[1] +
            (b1_A * bG2_B + b1_B * bG2_A) * pk_mult_symbolic[3] +
            (b1_A * (bG2_B + 0.4 * bGamma3_B) + b1_B * (bG2_A + 0.4 * bGamma3_A)) * pk_mult_symbolic[6] +
        ...
        derivatives = {}
        for param in [b1_A, b1_B, b2_A, b2_B, bG2_A, bG2_B, bGamma3_A, bGamma3_B, bNabla2_A, bNabla2_B, c0, c2]:
    ...
    def get_analytic_deriv_rsd(self):
        ...
        """
        # Define symbolic variables for bias and stochastic parameters
        b1_A, b2_A, bG2_A, bGamma3_A, cct_2_0_A, cct_2_2_A, cct_2_4_A, cct_4_4_A, cct_4_6_A = sp.symbols('b1_A b2_A bG2_A bGamma3_A cct_2_0_A cct_2_2_A cct_2_4_A cct_4_4_A cct_4_6_A')
        b1_B, b2_B, bG2_B, bGamma3_B, cct_2_0_B, cct_2_2_B, cct_2_4_B, cct_4_4_B, cct_4_6_B = sp.symbols('b1_B b2_B bG2_B bGamma3_B cct_2_0_B cct_2_2_B cct_2_4_B cct_4_4_B cct_4_6_B')
        cshot, cst_0_0, cst_0_2, cst_2_2, h_loc, nbarmix, knormh_loc, knorm_loc, k_loc, fz = sp.symbols('cshot cst_0_0 cst_0_2 cst_2_2 h_loc nbarmix knormh_loc knorm_loc k_loc fz')
#    
#        # Define symbolic multipole power spectrum terms
        ...
        # Add stochastic terms
        Pst_0 = (cshot + cst_0_0 + cst_0_2 * k2ratio + cst_2_2 * fz * k2ratio / 3.) / nbarmix
        Pst_2 = (2. * cst_2_2 * fz * k2ratio / 3.) / nbarmix
        Pst_4 = 0.
    
        # Compute multipoles
        monopole = (
            pk_mult_symbolic[15] + pk_mult_symbolic[21]
            + (b1_A + b1_B) / 2. * (pk_mult_symbolic[16] + pk_mult_symbolic[22])
            + b1_A * b1_B * (pk_mult_symbolic[17] + pk_mult_symbolic[23])
            + 0.25 * b2_A * b2_B * pk_mult_symbolic[1]
            + (b1_A * b2_B + b1_B * b2_A) / 2. * pk_mult_symbolic[30]
            + (b2_A + b2_B) / 2. * pk_mult_symbolic[31]
            + (b1_A * bG2_B + b1_B * bG2_A) / 2. * pk_mult_symbolic[32]
            + (bG2_A + bG2_B) / 2. * pk_mult_symbolic[33]
            + (b2_A * bG2_B + b2_B * bG2_A) / 2. * pk_mult_symbolic[4]
            + bG2_A * bG2_B * pk_mult_symbolic[5]
            + (bG2_A + 0.4 * bGamma3_A) * (b1_B * pk_mult_symbolic[7] + pk_mult_symbolic[8])
            + (bG2_B + 0.4 * bGamma3_B) * (b1_A * pk_mult_symbolic[7] + pk_mult_symbolic[8])
        ) * h_loc**3 + Pct_0_AB + Pct_0_BA + Pst_0
    
        quadrupole = (
            pk_mult_symbolic[18] + pk_mult_symbolic[24]
            + (b1_A + b1_B) / 2. * (pk_mult_symbolic[19] + pk_mult_symbolic[25])
            + b1_A * b1_B * pk_mult_symbolic[26]
            + (b1_A * b2_B + b1_B * b2_A) / 2. * pk_mult_symbolic[34]
            + (b2_A + b2_B) / 2. * pk_mult_symbolic[35]
            + (b1_A * bG2_B + b1_B * bG2_A) / 2. * pk_mult_symbolic[36]
            + (bG2_A + bG2_B) / 2. * pk_mult_symbolic[37]
            + (bG2_A + 0.4 * bGamma3_A) * pk_mult_symbolic[9]
            + (bG2_B + 0.4 * bGamma3_B) * pk_mult_symbolic[9]
        ) * h_loc**3 + Pct_2_AB + Pct_2_BA + Pst_2
    
        hexadecapole = (
            pk_mult_symbolic[20] + pk_mult_symbolic[27]
            + (b1_A + b1_B) / 2. * pk_mult_symbolic[28]
            + b1_A * b1_B * pk_mult_symbolic[29]
            + (b2_A * b2_B) / 2 * pk_mult_symbolic[38]
            + (bG2_A + bG2_B) / 2. * pk_mult_symbolic[39]
        ) * h_loc**3 + Pct_4_AB + Pct_4_BA + Pst_4
        ...
        derivatives_2 = {}
        derivatives_4 = {}
        for param in [b1_A, b2_A, bG2_A, bGamma3_A, cct_2_0_A, cct_2_2_A, cct_2_4_A, cct_4_4_A, cct_4_6_A, b1_B, b2_B, bG2_B, bGamma3_B, cct_2_0_B, cct_2_2_B, cct_2_4_B, cct_4_4_B, cct_4_6_B, cst_0_0, cst_0_2, cst_2_2]:
    ...
    def eval_derivative(self, paramname, AorB, h, bias_setA, bias_setB, stoch_set, nbarA_loc, nbarB_loc):
        """
        ...
        if (self.RSD == False): 
          # Unpack bias parameters
          b1_A_value, b2_A_value, bG2_A_value, bGamma3_A_value, bNabla2_A_value = bias_setA
          b1_B_value, b2_B_value, bG2_B_value, bGamma3_B_value, bNabla2_B_value = bias_setB
          ...
          if (paramname == 'b1_'): variable = [b1_A, b1_B]
          if (paramname == 'b2_'): variable = [b2_A, b2_B]
          if (paramname == 'bG2_'): variable = [bG2_A, bG2_B]
          if (paramname == 'bGamma3_'): variable = [bGamma3_A, bGamma3_B]
          if (paramname == 'bNabla2_'): variable = [bNabla2_A, bNabla2_B]
          if (paramname == 'c0_'): variable = [c0,c0]
          if (paramname == 'c2_'): variable = [c2,c2]
          ...
          for kidx, k_value in enumerate(self.k_loc):
              ...
              pk_subs = {pk_mult_symbolic[i]: eval_array[i, kidx] for i in range(len(pk_mult_symbolic))}
        ...
        # RSD part
          ...
          # Unpack bias parameters
          b1_A_value, b2_A_value, bG2_A_value, bGamma3_A_value, cct_2_0_A_value, cct_2_2_A_value, cct_2_4_A_value, cct_4_4_A_value, cct_4_6_A_value = bias_setA
          b1_B_value, b2_B_value, bG2_B_value, bGamma3_B_value, cct_2_0_B_value, cct_2_2_B_value, cct_2_4_B_value, cct_4_4_B_value, cct_4_6_B_value = bias_setB
          ...
          if (paramname == 'bG2_'): variable = [bG2_A, bG2_B]
          if (paramname == 'bGamma3_'): variable = [bGamma3_A, bGamma3_B]
          if (paramname == 'cct_2_0_'): variable = [cct_2_0_A, cct_2_0_B]
          if (paramname == 'cct_2_2_'): variable = [cct_2_2_A, cct_2_2_B]
          if (paramname == 'cct_2_4_'): variable = [cct_2_4_A, cct_2_4_B]
          if (paramname == 'cct_4_4_'): variable = [cct_4_4_A, cct_4_4_B]
          if (paramname == 'cct_4_6_'): variable = [cct_4_6_A, cct_4_6_B]
          if (paramname == 'cst_0_0_'): variable = [cst_0_0,cst_0_0]
          if (paramname == 'cst_0_2_'): variable = [cst_0_2,cst_0_2]
          if (paramname == 'cst_2_2_'): variable = [cst_2_2,cst_2_2]
